...
To verify you must use `.pct verify <screenshot link> <RSN>`
...
Syntax: `.inb <price> <screenshot link> <item>`
...
* `screenshot link` indicates a link to the screenshot showing the trade. [How to use discord as an image host](#Using-Discord-as-an-image-host)
...
Synax: `.pct record <type> <price> <screenshot link> <date> <time> <item>`
...
Users will often ignore the `screenshot link` parameter and upload a file instead. The `screenshot link` parameter cannot be empty and must include a link. Discord can however be used as an image host:
